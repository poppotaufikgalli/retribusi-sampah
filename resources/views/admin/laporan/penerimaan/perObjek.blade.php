...
@section('content')
    <div class="container-fluid px-4">
        <div class="card mb-4">
            <div class="card-body">
                <form method="POST" action="{{route('laporan.penerimaan.objek')}}">
                    @csrf
                    <div class="row mb-1">
                    ...
                    <div class="row mb-1">
                        <label class="col-sm-2">Objek Retribusi</label>
                        <div class="col-sm-6">
                            <select class="form-control form-control-sm" id="sobjek" name="sobjek">
                            @if(isset($objek_retribusi))
                                <option value="" selected>Semua Objek Retribusi</option> 
                                @foreach($objek_retribusi as $key => $value)
                                    <option value="{{$value->id}}" {{isset($sobjek) && $sobjek == $value->id ? 'selected' : ''}}>{{$value->nama}}</option>
                                @endforeach
                            @endif
                            </select>
                        </div>
                    ...
                    <div class="row mb-1">
                        <div class="col-sm-10 offset-sm-2">
                            ...
                            <a href="{{route('laporan.penerimaan.objek')}}" class="btn btn-sm btn-secondary">Reset</a>
                        </div>
                    </div>
                </form>
                <hr>
                        ...
                        topStart: {
                            buttons: [
                                'excelHtml5', 
                                {
                                    extend: 'pdfHtml5',
                                    pageSize: 'A4',
                                    title: 'Laporan Penerimaan Per Objek Retribusi',
                                },
                            ]
                        },
                        topEnd: 'search',
